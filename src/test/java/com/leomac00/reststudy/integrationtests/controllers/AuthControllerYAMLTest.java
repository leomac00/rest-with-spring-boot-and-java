...
class AuthControllerYAMLTest extends AbstractIntegrationTests {
    ...
    @Test
    @Order(1)
    void authorizationShouldReturnYAMLFormat() throws JsonProcessingException, JsonMappingException {
        AccountCredentialsVO credentials = new AccountCredentialsVO("admin", "admin123");

        var accessTokenYAML = given()
                .basePath("/auth")
                .port(TestConfigs.SERVER_PORT)
                .contentType(TestConfigs.CONTENT_TYPE_JSON)
                .header(TestConfigs.HEADER_PARAM_ACCEPT, TestConfigs.CONTENT_TYPE_YAML)
                .body(credentials)
                .when()
                .post()
                .then()
